...
int CHANNEL_remove_user(CHANNEL_conn* channel, client* clt) 
{
    ...
    for(auto arrived : channel->arrived) 
    {
        if(arrived->second == clt->nickname) 
        {
            channel->arrived.erase(arrived);
            break;
        }
    }
...
int CHANNEL_add_user(CHANNEL_conn* channel, client* clt) 
{
    ...
    clt->channel = channel->name_channel;

    return SUCCESS;
}

CHANNEL_conn* CHANNEL_join(string name_channel, client* clt) 
{    
    ...
    {
        chanel = conn_criar_CHANNEL(name_channel, clt);
        CHANNEL_broadcast(channel, clt, MESSAGE_NEW_CHANNEL, "");
    } else 
    {
        channel = CHANNEL_add_user(it_channel, clt);
        CHANNEL_broadcast(channel, clt, MESSAGE_JOIN_CHANNEL, "");
    }
}
 ...
 * 
 */
void CHANNEL_send_message_all(CHANNEL_conn* channel, client* clt, char* buffer)
{
...
int CHANNEL_broadcast(CHANNEL_conn* channel, client* clt, int type, char* buffer) 
{
    ...
    switch (type) 
    {
        ...
        case MESSAGE_JOIN_CHANNEL:
            ...
            CHANNEL_send_message_one()
            
            sprintf(msg_buffer, "SERVER << /join RPL_NAMREPLY %s +%s", channel->name_channel, clt->nickname);
            CHANNEL_send_message_one()

            sprintf(msg_buffer, "SERVER << /channelmsg RPL_NAMREPLY %s +%s", channel->name_channel, clt->nickname);
